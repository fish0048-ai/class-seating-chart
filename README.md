...
- 加分／扣分（可復原）
- 成績簿、學期統計、Excel 匯入匯出
- 統計圖表：上課加扣、平時考試、段考分開統計（班級／全校／個人）
- 模擬考能力等級（教師模式「模擬考」分頁；上課座位卡不顯示）
- 教學日誌（各班目前進度上到哪）
...
| 分組上課 | 上課模式按「分組」：設每組人數，可隨機或手動。再按「依分組排座位」會讓同一組坐在一起。勾「套用整組」或點側欄小組，加扣會進每位組員的平時成績。小組因誰被扣分，在教師模式 → 成績統計表可看 |
| 實驗室上課 | 切到「實驗室模式」：固定 6 組、座位六個區塊。可依座號或隨機分組，把學生拖到別區即可換組 |
| 用 Excel 改本班名單 | 設定 → 下載本班 Excel → 改完再上傳 |
| 保險備份 | 教師模式可下載 JSON 備份 |
| 看模擬考能力等級 | 設定與上傳 → 選擇學力診斷統計表 → 教師模式「模擬考」。上課座位卡不會標 |
| 記各班教學進度 | 教師模式 → 教學日誌：填目前進度後按儲存。同一天再存會覆蓋當天那筆 |
